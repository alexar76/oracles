...
from platon.randomness import Beacon, draw_randomness, verify_randomness
...
CAPABILITIES: list[dict[str, Any]] = [
    ...
    {
        "name": "platon.verify",
        "capability_id": "platon.verify@v1",
        "product_id": "prod-platon",
        "description": (
            "Verify a Platon chaos-VRF draw: checks the Ed25519 signature over "
            "random_hex + reproducibility proof without re-running the engine."
        ),
        "input_schema": {
            "type": "object",
            "required": ["random_hex", "proof", "signature"],
            "properties": {
                ...
                "public_key": {
                    "type": "string",
                    "description": (
                        "IGNORED. Kept for backward compatibility only. This capability "
                        "verifies against THIS oracle's own signing key — a caller-supplied "
                        "key would let the answer be about a draw the oracle never made. "
                        "Read the oracle's key from /.well-known/ai-market.json if you want "
...
_HANDLERS = {
    ...
    # The key is OURS, pinned, and any `public_key` in the request is ignored.
    #
    # This used to pass `d.get("public_key")`, and `verify_randomness` falls back to
    # `signature.public_key` — the key inside the caller's own document. Both were therefore
    # caller-controlled, so the capability answered "is this document self-consistent under
    # whatever key it carries", which anyone can make come out True by signing their own
    # draw with their own key. Its description promises something else entirely: "Verify a
    # PLATON chaos-VRF draw". And the answer travels inside a Platon-signed receipt, so a
    # downstream consumer reads a `valid: true` as this oracle vouching for the draw.
    "platon.verify@v1": lambda d: {
        "valid": verify_randomness(
            {"random_hex": d["random_hex"], "proof": d["proof"], "signature": d["signature"]},
            _signer.public_key_b64,
        )
    },
...
#: Serialized bytes of caller-supplied telemetry that may reach the witness prompt.
#: platon.oracle@v1 interpolates it into the LLM user message verbatim, so without a cap
#: the declared free-form ``{"type": "object"}`` input was an unbounded prompt-injection
#: and token-spend channel on the operator's provider key. platon.ask@v1 has had the same
#: firewall + 500-char clamp since it shipped (see platon/ask.py); this branch skipped it.
_MAX_TELEMETRY_BYTES = 2048


def _sanitize_telemetry(raw: Any) -> dict[str, Any]:
    """Keep scalar telemetry fields, bounded in count, length and total size."""
    ...
    for key, value in list(raw.items())[:32]:
        ...
        if not isinstance(value, str):
            continue  # nested structures are not telemetry; drop rather than relay
        text = value[:256]
        blocked = rejection_reason_if_blocked(text)
        if blocked:
            raise ValueError(f"telemetry field {name!r} rejected: {blocked}")
        out[name] = text
    if len(json.dumps(out, default=str).encode("utf-8")) > _MAX_TELEMETRY_BYTES:
        raise ValueError(
            f"telemetry exceeds {_MAX_TELEMETRY_BYTES} bytes once normalized"
        )
    return out
...
async def _run_capability(capability_id: str, input_data: dict[str, Any]) -> Any:
    if capability_id == "platon.oracle@v1":
        ...
        supplied = input_data.get("telemetry")
        telem = _sanitize_telemetry(supplied) if supplied else engine.telemetry()
        if "event" in input_data:
            event = input_data["event"]
            if not isinstance(event, str):
                raise ValueError("event must be a string")
            telem["event"] = _sanitize_telemetry({"event": event})["event"]
