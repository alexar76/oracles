    ...
    <div className="app" data-testid="platon-app">
      ...
      <main className="main">
        <div className="canvas-wrap" data-testid="shadow-canvas">
          ...
          />
          <div className="projection-hint projection-hint-desktop">{t.projectionHint}</div>
          <details className="projection-hint-details">
            <summary>{t.projectionHintSummary}</summary>
            <p>{t.projectionHint}</p>
          </details>
        </div>
      ...
      <footer className="footer">
        <span className="footer-primary">
          <span className={`status-dot${connected ? "" : " offline"}`} />
          {connected ? t.live : t.connecting}
          <span className="footer-tick"> · tick {telemetry?.tick ?? 0}</span>
        </span>
        <span className="footer-extra">
          · {telemetry?.viewers ?? 0} {t.projections} · {agentInvokes.length} {t.agentCalls}
        </span>
        <span className="footer-secondary">{t.footerRight}</span>
      </footer>
    </div>
  );
}
