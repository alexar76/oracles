...
had it via `server_nonce`), with a signed `entropy_commitment`. The entropy is
**committed and then revealed in the proof** (field `entropy`), so the output is
unpredictable *before* issuance (the entropy is secret until then) and, once
issued, independently **verifiable**: `verify_randomness` / `verify_beacon_chain`
open the commitment (`H(entropy) == entropy_commitment`) and re-derive `random_hex`
from it. A signed-but-fabricated output (operator grinding, or an unopened
commitment) is rejected — a signature alone, which the operator can always
produce, is no longer sufficient. (Being reproducible from the *revealed* entropy
is the point: that is what makes a beacon draw checkable, not "trust me".)
