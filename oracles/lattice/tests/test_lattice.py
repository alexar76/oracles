...
class TestHaltonMath:
    ...
    def test_radical_inverse_never_reaches_one(self):
        # Regression: for very large indices the floating-point sum of
        # 1 - base^-k rounds up to exactly 1.0 (e.g. n = 2^54 - 1 in base 2),
        # which would break the documented [0, 1) invariant. It must stay < 1.
        for k in range(50, 65):
            v = halton.radical_inverse(2**k - 1, 2)
            assert 0.0 <= v < 1.0, f"phi_2(2^{k}-1) = {v!r} escaped [0,1)"
        # base 3 all-digit index: n = 3^k - 1 -> 1 - 3^-k
        for k in range(30, 45):
            v = halton.radical_inverse(3**k - 1, 3)
            assert 0.0 <= v < 1.0, f"phi_3(3^{k}-1) = {v!r} escaped [0,1)"

    def test_halton_upper_bound_holds_for_huge_skip(self):
        # The clamp must hold through the public surface too: `skip` is
        # unbounded, so a large skip drives the index into the danger zone.
        skip = (2**54 - 1) - 1  # first index becomes 2^54 - 1
        pts = halton.halton(1, dim=1, skip=skip)
