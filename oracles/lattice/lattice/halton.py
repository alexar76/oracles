...
# Largest IEEE-754 double strictly below 1.0 (== 1 - 2^-53). The radical inverse
# is mathematically in [0, 1), but for very large indices the true value
# 1 - base^-k has no exact double once k > 53 and can round up to exactly 1.0
# (e.g. phi_2(2^54 - 1)). We clamp to this value to keep the [0, 1) invariant.
...
def radical_inverse(n: int, base: int) -> float:
    ...
    Reflect the base-`base` digits of `n` around the radix point using the
    standard accumulation (divide a running fraction by the base each digit).
    The result is exact for the index ranges we serve; for very large indices
    the true value 1 - base^-k can round up to exactly 1.0 in double precision,
    so the return value is clamped to the largest double below 1.0 to preserve
    the [0, 1) invariant.
    """
    if base < 2:
    ...
    while i > 0:
        ...
        i //= base
        f /= base
    # Guard the [0, 1) upper bound: floating-point rounding of 1 - base^-k can
    # reach exactly 1.0 for very large indices. Clamp to the nearest double < 1.
    if result >= 1.0:
