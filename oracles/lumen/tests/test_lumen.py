...
from lumen import capabilities, pagerank
...
def test_pagerank_docstring_has_no_editing_artifact():
    # Regression for the garbled "...the trust *received* by — no: each source's..."
    # editing artifact that used to sit in the module docstring.
    doc = pagerank.__doc__
    assert "received" not in doc
    assert "— no:" not in doc
    # The corrected sentence describes source-column normalization.
    assert "each source column" in doc.replace("\n", " ")


class TestGraphCommitment:
    def test_commitment_is_deterministic_and_order_independent(self):
        e1 = [[0, 1, 2.0], [1, 2, 1.0], [2, 0, 3.0]]
        e2 = [[2, 0, 3.0], [0, 1, 2.0], [1, 2, 1.0]]  # same edges, shuffled
        c1 = pagerank.graph_commitment(3, e1)
        c2 = pagerank.graph_commitment(3, e2)
        assert c1 == c2
        assert c1.startswith("sha256:")

    def test_commitment_aggregates_parallel_edges(self):
        # Splitting one edge into two half-weight edges is the same weighted graph.
        combined = pagerank.graph_commitment(3, [[0, 1, 2.0]])
        split = pagerank.graph_commitment(3, [[0, 1, 1.0], [0, 1, 1.0]])
        assert combined == split

    def test_commitment_changes_when_graph_changes(self):
        base = pagerank.graph_commitment(3, [[0, 1, 1.0]])
        assert base != pagerank.graph_commitment(3, [[0, 1, 1.5]])  # different weight
        assert base != pagerank.graph_commitment(3, [[0, 2, 1.0]])  # different endpoint

    def test_commitment_rejects_bad_graph(self):
        with pytest.raises(ValueError):
            pagerank.graph_commitment(2, [[0, 5, 1.0]])


class TestVerifyBindsCommitment:
    @staticmethod
    def _scores(nodes, edges):
        return pagerank.pagerank(nodes, edges)["scores"]

    def test_matching_commitment_accepted(self):
        nodes = 4
        edges = [[0, 1, 1.0], [1, 2, 1.0], [2, 3, 1.0], [3, 0, 1.0]]
        scores = self._scores(nodes, edges)
        commit = pagerank.graph_commitment(nodes, edges)
        out = capabilities._verify(
            {"nodes": nodes, "edges": edges, "scores": scores, "graph_commitment": commit}
        )
        assert out["valid"] is True
        assert out["graph_commitment"] == commit

    def test_wrong_commitment_rejected_even_when_scores_match(self):
        # Regression: the scores ARE the correct fixed point for these edges, but the
        # caller bound the claim to a DIFFERENT graph. Before the fix graph_commitment
        # was declared in the schema yet never read, so this returned valid=True.
        nodes = 4
        edges = [[0, 1, 1.0], [1, 2, 1.0], [2, 3, 1.0], [3, 0, 1.0]]
        scores = self._scores(nodes, edges)
        bogus = pagerank.graph_commitment(nodes, [[0, 1, 9.0]])
        out = capabilities._verify(
            {"nodes": nodes, "edges": edges, "scores": scores, "graph_commitment": bogus}
        )
        assert out["valid"] is False
        assert out["error"] == "graph_commitment mismatch"

    def test_verify_without_commitment_still_works(self):
        nodes = 3
        edges = [[0, 2, 1.0], [1, 2, 1.0]]
        scores = self._scores(nodes, edges)
        out = capabilities._verify({"nodes": nodes, "edges": edges, "scores": scores})
        assert out["valid"] is True
        assert "graph_commitment" in out


class TestScorePercentile:
    def test_top_node_percentile_below_100(self):
        # Everyone trusts node 3 -> it is rank 1; its percentile must stay in [0, 100).
        # Regression: the old ((n - rank + 1) / n) * 100 gave the rank-1 node 100.0.
        edges = [[0, 3, 1.0], [1, 3, 1.0], [2, 3, 1.0], [4, 3, 1.0]]
        out = capabilities._score({"nodes": 5, "edges": edges, "target_node": 3})
        assert out["rank"] == 1
        assert 0.0 <= out["percentile"] < 100.0

    def test_percentile_in_range_for_every_node(self):
        edges = [[0, 3, 1.0], [1, 3, 1.0], [2, 3, 1.0], [4, 3, 1.0]]
        for target in range(5):
            out = capabilities._score({"nodes": 5, "edges": edges, "target_node": target})
            assert 0.0 <= out["percentile"] < 100.0
