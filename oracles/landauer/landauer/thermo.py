...
at least
    ...
    E_min = k_B · T · ln 2     (≈ 2.87 zJ at T = 300 K),
...
2. **Per-gate erasure count.** Each gate's irreversible bit-loss is read from a small,
   ...
   ``k - 1`` bits per bit-lane, i.e. ``w · (k - 1)`` bits for a ``w``-bit-wide reduction
   (``w`` defaults to 1 for a scalar wire); an explicit ``erase`` of a ``w``-bit register
   destroys ``w`` bits. Reversible gates destroy ``0`` bits.
...
# state space shrinks by a factor 2^(k-1).) A reduction over w-bit-wide wires runs one
# such collapse per bit-lane, so it destroys w·(k-1) bits (w defaults to 1).
REDUCTIONS = {
...
def _gate_erased_bits(gate: str, fan_in: int, width: int | None) -> int:
    ...
    A declared bit-width ``w`` scales the erasure per bit-lane (``w`` defaults to 1, a
    scalar wire):

    - reversible gate                       → 0
    - width-eraser (erase/reset/measure …)  → declared width w (a full w-bit overwrite)
    - boolean reduction with fan-in k       → w · max(k - 1, 0)
    - unknown gate                          → treated as a worst-case k-input reduction
                                              (w · (k-1) bits) so audits fail *safe*
                                              (over-counting cost rather than hiding it).
    """
    g = gate.strip().lower()
    if g in REVERSIBLE:
        return 0
    # Declared bit-width scales every non-reversible gate's erasure (default 1).
    ...
    if g in WIDTH_ERASERS:
        # a full overwrite of a w-bit register destroys exactly w bits.
        return w
    # reductions and unknown gates collapse k inputs to 1 output on each of the w
    # bit-lanes → w·(k-1) erased bits. Unknown gates use this same conservative
    # reduction model so an unrecognised op can never silently hide its cost.
    k = fan_in
    return w * max(k - 1, 0)
...
def verify(
    ...
    # round-trips through JSON. One bit at 300 K is ~2.871e-21 J, so we compare with a
