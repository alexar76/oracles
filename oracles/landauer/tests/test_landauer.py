...
def and2_wide32():
    # a 32-bit-wide AND of two 32-bit input registers: each of the 32 bit-lanes runs
    # one 2-input reduction → 32 · (2 - 1) = 32 erased bits (not a scalar 1).
    return [
        {"id": "a", "gate": "input", "width": 32},
        {"id": "b", "gate": "input", "width": 32},
        {"id": "g", "gate": "and", "inputs": ["a", "b"], "width": 32},
        {"id": "out", "gate": "output", "inputs": ["g"], "width": 32},
    ]
...
class TestAuditPhysics:
    ...
    def test_landauer_floor_value_at_300k(self):
        # one bit at 300 K must equal k_B · 300 · ln2 ≈ 2.870978885e-21 J (≈ 2.87 zJ).
        out = th.audit(and2(), temperature_k=300.0)
        expected = th.K_B * 300.0 * math.log(2.0)
        assert abs(out["energy_floor_j"] - expected) < 1e-30
        # tight literal check: locks the magnitude at ~2.871 zJ (not the old ~2.85 zJ).
        assert abs(out["energy_floor_j"] - 2.870978885078724e-21) < 1e-27
...
# --- declared wire width honoured -------------------------------------------------

class TestWireWidth:
    def test_wide_reduction_scales_with_width(self):
        # regression: a width-w reduction erases w·(k-1) bits per its declared lanes,
        # not a scalar 1. Before width was honoured this returned 1 and under-reported
        # the true 32-bit Landauer floor by 32×.
        out = th.audit(and2_wide32())
        assert out["irreversible_bits"] == 32

    def test_scalar_reduction_defaults_to_width_one(self):
        # a reduction with no declared width keeps the scalar-per-op default (w = 1).
        assert th.audit(and2())["irreversible_bits"] == 1

    def test_wide_reduction_energy_scales(self):
        wide = th.audit(and2_wide32(), temperature_k=300.0)["energy_floor_j"]
        scalar = th.audit(and2(), temperature_k=300.0)["energy_floor_j"]
        assert abs(wide - 32.0 * scalar) < 1e-30

    def test_verify_rejects_underreported_wide_reduction(self):
        # claiming the scalar 1-bit cost for a 32-bit-wide AND under-reports the floor
        # and must now be rejected (it would have been accepted before the width fix).
        v = th.verify(and2_wide32(), irreversible_bits=1)
        assert v["valid"] is False
        assert v["recomputed_irreversible_bits"] == 32

    def test_wide_reduction_width_capped(self):
        # an over-wide reduction is rejected by the same MAX_WIDTH guard as erasers.
        ops = [
            {"id": "a", "gate": "input"},
            {"id": "b", "gate": "input"},
            {"id": "g", "gate": "and", "inputs": ["a", "b"], "width": th.MAX_WIDTH + 1},
        ]
        with pytest.raises(ValueError):
            th.audit(ops)
