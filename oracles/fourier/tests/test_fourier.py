...
# --- 2026-09 re-audit: the cost estimate must see the graph the solver will build -----
#
# `min(len(d.get("nodes") or []) or _n_from_edges(d), MAX_NODES)` — because of the `or`, any
# non-empty `nodes` list SUPPRESSED the edge-derived count. But spectral.canonical_graph
# builds its label set as the UNION of `nodes` and every edge endpoint, so `nodes: ["n0"]`
# made the estimator price a 1-node graph while the solver ran a 400-node one. Cost is
# O(n^3) here (numpy.linalg.eigh on a dense Laplacian), so the under-charge is enormous and
# the CPU budget it feeds becomes decorative.

def test_a_one_element_nodes_list_cannot_hide_a_large_edge_set():
    from fourier.capabilities import SPEC

    cap = next(c for c in SPEC.capabilities if c.capability_id == "fourier.spectrum@v1")
    edges = [[f"n{i}", f"n{(i + 1) % 400}"] for i in range(400)]
    honest = cap.estimate_cost_ms({"edges": edges})
    hidden = cap.estimate_cost_ms({"nodes": ["n0"], "edges": edges})
    assert hidden == honest, (
        f"declaring nodes:['n0'] priced the call at {hidden} instead of {honest}"
    )


def test_the_estimate_counts_the_union_not_just_the_larger_side():
    """nodes and edge endpoints can be disjoint; the solver's graph is both."""
    from fourier.capabilities import SPEC

    cap = next(c for c in SPEC.capabilities if c.capability_id == "fourier.spectrum@v1")
    only_nodes = cap.estimate_cost_ms({"nodes": [f"a{i}" for i in range(50)]})
    only_edges = cap.estimate_cost_ms({"edges": [[f"b{i}", f"b{i+1}"] for i in range(50)]})
    both = cap.estimate_cost_ms({
        "nodes": [f"a{i}" for i in range(50)],
        "edges": [[f"b{i}", f"b{i+1}"] for i in range(50)],
    })
    assert both > max(only_nodes, only_edges)
