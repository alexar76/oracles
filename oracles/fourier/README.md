...
**Trustless verification (a bound, not an equality).** Given the graph and a claimed eigenpair `(λ, x)`, a verifier checks `‖L x − λ x‖ / ‖x‖ ≤ tol` (it really is an eigenpair) and that `x ⟂` the trivial λ₁ eigenvector (a genuinely *non-trivial* mode). That **O(E)** certificate proves λ is a valid non-trivial Laplacian eigenvalue, and since every non-trivial eigenvalue is `≥ λ₂` (Courant–Fischer), it certifies `λ ≥ λ₂` — an **upper bound** on the algebraic connectivity. It does **not** prove `λ = λ₂`: no single-eigenpair check can show λ is the *smallest* non-trivial eigenvalue without the full O(n³) eigendecomposition. `fourier.spectrum@v1` runs that decomposition and returns the exact λ₂.
...
| `fourier.verify@v1` | `$0.001` | cheap O(E) non-trivial-eigenpair certificate (an upper bound `λ ≥ λ₂`, not a proof `λ = λ₂`): `{valid, residual, orthogonality, is_eigenpair, is_nontrivial, lambda2_upper_bound, graph_commitment}` |
...
// output
{ "valid": true, "residual": 1e-12, "orthogonality": 0.0, "is_eigenpair": true,
  "is_nontrivial": true, "lambda2_upper_bound": 0.298, "graph_commitment": "…" }
```

`valid` is true iff the residual is within tolerance (`≤ tol`) **and** the vector is non-trivial — orthogonal to the all-ones / `d^{1/2}` λ₁ mode to within the **same caller-supplied `tol`** (no hidden threshold). A `valid` certificate proves `λ` is a genuine non-trivial eigenvalue, hence `λ ≥ λ₂` (reported as `lambda2_upper_bound`). It is an **upper bound** on the algebraic connectivity, **not** a proof that `λ` is the Fiedler value itself.
