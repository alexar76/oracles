...
**Verification is cheap and trustless — but it is a bound, not an equality.** Given the
graph and a claimed eigenpair ``(λ, x)``, a verifier checks the eigen-relation
``‖L x − λ x‖ / ‖x‖ ≤ tol`` (it really is an eigenpair) and that ``x ⟂ 1`` (orthogonal
to the all-ones λ₁ eigenvector — a genuinely *non-trivial* mode, not the constant one).
Together these certify that λ is a valid **non-trivial** Laplacian eigenvalue, and since
every non-trivial eigenvalue is ``≥ λ₂`` (Courant–Fischer), λ is a certified **upper
bound** on the algebraic connectivity: ``λ ≥ λ₂``. This ``O(E)`` check does **not** prove
``λ = λ₂`` — a single-eigenpair test cannot show λ is the *smallest* non-trivial
eigenvalue without redoing the full ``O(n³)`` eigendecomposition. ``analyze`` computes
and returns the true λ₂; the trustless certificate only bounds it from above.
"""
...
DEFAULT_K = 6
DEFAULT_TOL = 1e-6
# Loosest tolerance at which `verify` will still call a claim valid. The eigen-relation
# residual and the orthogonality ratio are both dimensionless and normalised, so a real
# eigenpair on a well-formed graph lands orders of magnitude below this; anything that
# needs a looser bar is not an eigenpair. Clamped rather than rejected so an existing
# caller passing a sloppy tol keeps working — it just cannot buy a false certificate.
MAX_TOL = 1e-3
...
def verify(
    ...
    * **orthogonality** ``|⟨x, 1⟩| / (‖x‖ √n)`` (combinatorial) or ``|⟨x, d^{1/2}⟩| /
      ...
      Must be ``≤ tol`` for ``x`` to count as a non-trivial mode (not the constant one).

    ``valid`` is true iff the residual is within tolerance (a genuine eigenpair) AND the
    vector is non-trivial (orthogonal to the λ₁ mode). What that certifies, honestly, is
    that **λ is a valid non-trivial Laplacian eigenvalue**, hence ``λ ≥ λ₂`` — an ``O(E)``
    **upper bound** on the algebraic connectivity (returned as ``lambda2_upper_bound``).
    It does **not** prove ``λ = λ₂``: no single-eigenpair check can show λ is the
    *smallest* non-trivial eigenvalue without the full ``O(n³)`` eigendecomposition, which
    ``analyze`` runs to report the true λ₂.
    """
    if laplacian_kind not in LAPLACIANS:
        raise ValueError(f"laplacian must be one of {LAPLACIANS}; got {laplacian_kind!r}")
    labels, _index, wedges, commitment = canonical_graph(nodes, edges)
    ...
    # A certificate is only meaningful at a TIGHT tolerance, so tol is clamped, not
    # trusted. Unclamped it was the whole certificate's undoing: tol=1e12 certified any
    # (λ, x) as a valid non-trivial eigenpair, and the response then reported
    # `lambda2_upper_bound = λ` — a mathematically false bound the caller chose. Passing
    # λ=0 that way certified that the customer's network is disconnected.
    # A caller may still tighten tol below the default; they may not loosen it past
    # MAX_TOL, above which "verified" would not mean anything.
    tol_requested = float(tol)
    tol_used = min(tol_requested, MAX_TOL)
    is_eigenpair = residual <= tol_used
    # Non-triviality bar honours the CALLER's tol (no hidden 1e-3 floor): a strict tol
    # must reject a vector that is measurably aligned with the trivial λ₁ mode.
    is_nontrivial = orthogonality <= tol_used
    ...
    result: dict[str, Any] = {
        ...
        "graph_commitment": commitment,
        # Report the bar actually applied, so a caller who asked for a loose tol can see
        # that it was not honoured rather than believing their own number was used.
        "tol": tol_used,
    }
    if tol_requested > tol_used:
        result["tol_requested"] = tol_requested
        result["note_tol"] = (
            f"requested tol {tol_requested:g} exceeds MAX_TOL {MAX_TOL:g} and was "
            f"clamped: a certificate at a loose tolerance certifies nothing"
        )
    # A verified non-trivial eigenpair proves λ ≥ λ₂ (Courant–Fischer), so λ is a
    # certified UPPER BOUND on the algebraic connectivity — NOT a proof that λ = λ₂.
    if valid:
        result["lambda2_upper_bound"] = float(lambda_)
    return result
