...
def _n_from_edges(d: dict[str, Any]) -> int:
    """Node count implied by the edge list, for cost estimation only.

    `cost_ms` is consulted BEFORE the handler runs, so it cannot use the canonical
    graph — it has to estimate from the raw request. Distinct endpoint labels over the
    first two positions of each edge is exact for well-formed input and merely an
    over-estimate for malformed input, which is the safe direction for a budget.
    """
    seen: set[Any] = set()
    ...
    capabilities=[
        Capability(
            ...
            cost_ms=lambda d: min(
                5_000.0,
                (min(len(d.get("nodes") or []) or _n_from_edges(d), spectral.MAX_NODES) ** 3)
                / 640_000.0
                + 1.0,
            ),
