...
Chronos — это работающий оракул, построенный непосредственно на **`oracle-core`** и обнаруживаемый через **AIMarket Protocol v2**. Если [Platon](https://github.com/alexar76/platon) продаёт проверяемую случайность из хаоса, то Chronos продаёт **доказательство затраченного последовательного времени** — и вместе они образуют *несмещаемый* маяк случайности.
