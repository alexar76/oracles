...
def _aggregate(d: dict[str, Any]) -> dict[str, Any]:
    ...
    # `float(d.get("trim", 0.1))` threw TypeError on anything non-numeric — a dict, a list,
    # an explicit null — and an unhandled TypeError leaves the service as HTTP 500 with no
    # indication of which field was wrong. Say it.
    raw_trim = d.get("trim")
    if raw_trim is None:
        trim = 0.1
    else:
        try:
            ...
            raise ValueError(
                f"'trim' must be a number, got {type(raw_trim).__name__}"
            ) from None
    if not isinstance(values, (list, tuple)):
        raise ValueError(f"'values' must be an array, got {type(values).__name__}")
    return consensus.aggregate(list(values), trim)
