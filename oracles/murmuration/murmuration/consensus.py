...
# Tukey biweight tuning constant. With the consistent (×1.4826) MAD scale used by
# ``mad`` below, c=4.685 is the standard Holland–Welsch value giving ~95% asymptotic
# efficiency at the Gaussian (it matches e.g. R's ``MASS::rlm`` bisquare default),
# while still handing exactly zero weight to anything beyond c scaled-MAD units from
# the centre. (c=6.0 would push efficiency to ~98% but at the cost of robustness — it
# does *not* give the ~95% we claim.)
BIWEIGHT_C = 4.685
_EPS = 1e-12
