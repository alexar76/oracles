...
from murmuration.receipts import output_hash, verify_envelope
...
def _gaussian_efficiency(c: float, n: int = 200, trials: int = 3000, seed: int = 20260711) -> float:
    """Monte-Carlo estimate of the biweight's efficiency at a clean Gaussian.

    Efficiency = Var(sample mean) / Var(biweight); the mean is the minimum-variance
    unbiased estimator under Gaussian errors, so this ratio is ≤ 1 and equals the
    biweight's asymptotic efficiency in the large-n limit. Deterministic: PCG64 +
    the ziggurat normal sampler are stable across numpy versions.
    """
    ...
    for i in range(trials):
        ...
        means[i] = x.mean()
        bws[i] = consensus.biweight_location(x, c=c)
    return float(np.var(means, ddof=1) / np.var(bws, ddof=1))


class TestBiweightEfficiency:
    """Finding (a): the tuning constant must actually deliver the claimed ~95%."""

    def test_default_constant_is_standard_95pct_value(self):
        # Holland–Welsch value for the consistent (×1.4826) MAD scale used here.
        assert consensus.BIWEIGHT_C == pytest.approx(4.685)

    def test_default_gives_about_95pct_efficiency_not_98(self):
        # The shipped default must land near the claimed ~95% Gaussian efficiency...
        eff_default = _gaussian_efficiency(consensus.BIWEIGHT_C)
        assert 0.90 <= eff_default <= 0.965, eff_default
        # ...and be distinguishable from the old c=6.0, which gives ~98% (and so is
        # NOT the "~95%" the docs claim). This is what fails with the pre-fix c=6.0.
        eff_c6 = _gaussian_efficiency(6.0)
        assert eff_c6 > 0.965, eff_c6


class TestReceiptBindsResult:
    """Finding (b): the signed receipt must bind the actual output + input_hash."""

    async def _invoke(self, values):
        transport = ASGITransport(app=app)
        async with AsyncClient(transport=transport, base_url="http://t") as c:
            return (
                await c.post(
                    "/ai-market/v2/invoke",
                    json={"capability_id": "murmuration.aggregate@v1", "input": {"values": values}},
                )
            ).json()

    @pytest.mark.asyncio
    async def test_receipt_binds_output_and_verifies(self):
        env = await self._invoke([10.0, 11.0, 9.0, 10.5])
        key = app.state.protocol.signer.public_key_b64
        # The receipt now carries the bound hashes + a binding signature.
        receipt = env["receipt"]
        assert receipt["output_hash"] == output_hash(env["output"])
        assert receipt["input_hash"] == env["provenance"]["input_hash"]
        assert "binding" in receipt and receipt["binding"].get("value")
        # Full end-to-end verification succeeds for an untampered envelope.
        assert verify_envelope(env, key) is True
        # And the hub-compatible 7-field signature is preserved (regression guard).
        assert app.state.protocol.signer.verify_receipt(receipt) is True

    @pytest.mark.asyncio
    async def test_tampered_output_is_rejected(self):
        env = await self._invoke([10.0, 11.0, 9.0, 10.5])
        key = app.state.protocol.signer.public_key_b64
        assert verify_envelope(env, key) is True
        # A relaying agent under-reports / rewrites the consensus number.
        env["output"]["median"] = 0.0
        assert verify_envelope(env, key) is False

    @pytest.mark.asyncio
    async def test_tampered_output_with_patched_hash_still_rejected(self):
        # Attacker rewrites the output AND updates the receipt's output_hash to match,
        # trying to slip past the recompute check — the binding signature catches it.
        env = await self._invoke([1.0, 2.0, 3.0, 4.0, 100.0])
        key = app.state.protocol.signer.public_key_b64
        env["output"]["converged_value"] = 3.0  # was the outlier-pulled mean, 22.0
        env["receipt"]["output_hash"] = output_hash(env["output"])
        assert verify_envelope(env, key) is False

    @pytest.mark.asyncio
    async def test_forged_key_is_rejected_when_pinned(self):
        # Consumer pins the oracle's real key; a self-consistent envelope re-signed
        # under a different key must not verify against the pinned identity.
        env = await self._invoke([5.0, 6.0, 7.0])
        real_key = app.state.protocol.signer.public_key_b64
        assert verify_envelope(env, real_key) is True
        # Sanity: without pinning it still self-verifies (internal consistency only).
        assert verify_envelope(env) is True
        # Swap in an unrelated key -> pinned verification fails.
        from oracle_core.signing import Signer as _Signer
        import base64
