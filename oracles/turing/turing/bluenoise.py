...
def expected_uniform_nn_spacing(count: int) -> float:
    """Typical *nearest-neighbour* spacing scale for ``count`` uniform i.i.d. points
    in the unit square: ~ 0.5 / sqrt(count).

    This is the mean distance from a point to its closest neighbour (the density
    scale 1 / sqrt(count) up to a constant), NOT the expected *minimum* distance
    over all pairs. The global minimum is a much smaller quantity: with ~count^2
    pairs it scales as ~ 1 / count, i.e. roughly ``sqrt(count)`` times smaller than
    this value. Used only for docs / sanity comparisons; the tests measure the real
    uniform set directly.
    """
