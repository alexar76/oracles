...
class TestBlueNoise:
    ...
    def test_nn_spacing_is_nn_not_min(self):
        # Regression: the 0.5/sqrt(count) formula is the typical NEAREST-NEIGHBOUR
        # spacing, NOT the expected MINIMUM pairwise distance (which is ~sqrt(count)x
        # smaller). The old name/docstring called it "min distance", which is false.
        # The honest symbol must exist...
        assert hasattr(bluenoise, "expected_uniform_nn_spacing")
        # ...and the misleadingly-named old symbol must be gone.
        assert not hasattr(bluenoise, "expected_uniform_min_distance")

        n = 400
        formula = bluenoise.expected_uniform_nn_spacing(n)

        rng = np.random.default_rng(123)
        nn_means, true_mins = [], []
        for _ in range(12):
            uni = rng.random((n, 2))
            # Mean nearest-neighbour distance over all points.
            diff = uni[:, None, :] - uni[None, :, :]
            d2 = np.einsum("ijk,ijk->ij", diff, diff)
            np.fill_diagonal(d2, np.inf)
            nn_means.append(float(np.mean(np.sqrt(d2.min(axis=1)))))
            true_mins.append(_min_dist(uni))
        mean_nn = float(np.mean(nn_means))
        true_min = float(np.mean(true_mins))

        # The formula tracks the mean nearest-neighbour spacing (within 20%).
        assert math.isclose(formula, mean_nn, rel_tol=0.2), (formula, mean_nn)
        # It is NOT the minimum: the real min is several times smaller.
        assert true_min < 0.4 * formula, (true_min, formula)
