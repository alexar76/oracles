...
class TestVerify:
    ...
    def test_verify_accepts_a_seed_that_restates_the_committed_one(self):
        """A supplied seed is fine only as a redundant restatement of the committed one.

        This test used to pass `seed=` WITHOUT the nonce and assert valid is True,
        which blessed the grinding hole below. The seed must accompany the nonce it
        was derived from, and then it is merely confirmatory.
        """
        out = sp.cascade(grid(6), grains=2000, nonce="seedpath")
        v = sp.verify(
            grid(6), grains=2000, nonce="seedpath", seed=out["seed"],
            claimed_tau=out["tau"], claimed_topple_total=out["topple_total"],
        )
        assert v["valid"] is True
        assert v["seed"] == out["seed"]

    def test_verify_rejects_grindable_seed(self):
        """Security regression: a caller-chosen seed must not be replayed.

        Before the fix, verify() replayed whatever seed it was handed and certified
        the result — so a claimant could grind the driving schedule for a flattering
        tau, publish (graph, seed, tau), and have this verifier return valid: true.
        Confirmed exploitable against the live oracle. Sibling percola.verify already
        guarded the identical hole.
        """
        out = sp.cascade(grid(6), grains=2000, nonce="honest")
        v = sp.verify(
            grid(6), grains=2000, nonce="honest", seed="deadbeefdeadbeef",
            claimed_tau=out["tau"], claimed_topple_total=out["topple_total"],
        )
        assert v["valid"] is False
        assert "committed" in v.get("error", "")
        # The replay must have used the committed seed, not the attacker's.
        assert v["seed"] != "deadbeefdeadbeef"

    def test_verify_without_nonce_cannot_be_certified_by_a_bare_seed(self):
        """No nonce + a bare seed is the exact shape of the old attack payload."""
        v = sp.verify(
            grid(6), grains=2000, seed="deadbeefdeadbeef",
            claimed_tau=1.9567, claimed_topple_total=198,
        )
        assert v["valid"] is False
...
# --- 2026-09 re-audit: same `or` as fourier, plus two more uncounted label sources ----
#
# sandpile.canonical_config unions `nodes`, the KEYS of `capacities`, `sinks`, and every
# edge endpoint. The estimator read `len(nodes) or _n_from_edges(d)`, so a non-empty `nodes`
# suppressed the edges, and `capacities`/`sinks` were never counted at all.

def test_a_one_element_nodes_list_cannot_hide_a_large_edge_set():
    from ablation.capabilities import SPEC

    cap = next(c for c in SPEC.capabilities if c.capability_id.startswith("ablation.cascade"))
    edges = [[f"n{i}", f"n{(i + 1) % 300}"] for i in range(300)]
    honest = cap.estimate_cost_ms({"edges": edges})
    hidden = cap.estimate_cost_ms({"nodes": ["n0"], "edges": edges})
    assert hidden == honest, f"nodes:['n0'] priced the call at {hidden}, not {honest}"


def test_capacities_and_sinks_are_counted_as_nodes():
    from ablation.capabilities import SPEC

    cap = next(c for c in SPEC.capabilities if c.capability_id.startswith("ablation.cascade"))
    bare = cap.estimate_cost_ms({"edges": [["a", "b"]]})
    with_caps = cap.estimate_cost_ms({
        "edges": [["a", "b"]],
        "capacities": {f"c{i}": 1 for i in range(80)},
        "sinks": [f"s{i}" for i in range(20)],
    })
    assert with_caps > bare, "capacities/sinks add nodes the solver builds but nobody priced"
