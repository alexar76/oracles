...
def canonical_config(
    ...
    for i, lab in enumerate(labels):
        if lab in cap_in:
            ...
            # A topple removes `c` grains and sheds one per out-edge, so c < out-degree
            # makes toppling grain-CREATING: mass grows every generation instead of
            # dissipating. That is not a model of anything, and it is a cheap remote CPU
            # bomb — a 4000-node degree-10 graph with capacities of 1 amplifies tenfold
            # per generation and burns the whole MAX_TOPPLES budget in pure Python.
            # Reject rather than clamp: a supplied capacity that cannot dissipate is a
            # mis-specified model, and silently substituting a different one would make
            # the commitment describe a config the caller did not ask for.
            if c < len(out[i]):
                raise ValueError(
                    f"capacity for node {lab!r} is {c} but its out-degree is "
                    f"{len(out[i])}: a topple would emit more grains than it removes, "
                    f"so the pile can never relax (capacity must be >= out-degree)"
                )
            capacity.append(c)
        else:
...
def verify(
    ...
    # The driving schedule's seed is COMMITTED, not a free parameter: it must be
    # H(commitment || nonce). We always derive it, and a supplied `seed` is accepted
    # only as a redundant restatement that must equal the committed value.
    #
    # Replaying an arbitrary caller-chosen seed made this grindable: a claimant could
    # fish for a schedule yielding a flattering tau, publish (graph, seed, tau), and
    # this verifier would certify `valid: true`. Confirmed exploitable against the
    # live oracle before this guard. Sibling `percola.verify` already closed exactly
    # this hole (percola/percolation.py:305-323) — matching it here.
    used_nonce = str(nonce)
    committed_seed = seed_int(cfg["commitment"], used_nonce)
    seed_error: str | None = None
    if seed is not None:
        try:
            supplied_seed = int(str(seed), 16)
        except ValueError:
            supplied_seed = None
            seed_error = "seed is not valid hex"
        if seed_error is None and supplied_seed != committed_seed:
            seed_error = (
                "supplied seed does not equal H(commitment || nonce); the driving "
                "schedule's seed is committed, not caller-chosen"
            )
    seed_int_val = committed_seed
    ...
    if seed_error is not None:
        valid = False  # a seed that is not the committed one proves nothing

    return {
        "valid": bool(valid),
        **({"error": seed_error} if seed_error else {}),
        "config_commitment": cfg["commitment"],
