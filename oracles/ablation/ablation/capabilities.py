...
def _n_from_edges(d: dict[str, Any]) -> int:
    ...
    `cost_ms` runs BEFORE the handler, so it cannot consult the canonical config and has
    to estimate from the raw request. Over-estimating on malformed input is the safe
    ...
    capabilities=[
        Capability(
            ...
            cost_ms=lambda d: min(
                ...
                * max(1, min(len(d.get("nodes") or []) or _n_from_edges(d), sandpile.MAX_NODES))
                / 500.0,
            ),
