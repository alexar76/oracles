...
def _n_from_request(d: dict[str, Any]) -> int:
    """Node count of the configuration the solver will build, for cost estimation only.

    `sandpile.canonical_config` unions FOUR sources: `nodes`, the KEYS of `capacities`,
    `sinks`, and both endpoints of every edge. The estimator counted at most one of them:
    it read `len(nodes) or _n_from_edges(d)`, so a non-empty `nodes` short-circuited the
    edges (the same `or` bug as fourier) and `capacities`/`sinks` were never counted at all.
    A caller could therefore describe a large graph entirely through those fields and be
    priced for a one-node one.

    Over-estimating is the safe direction for a budget, so malformed entries that still
    name a label are counted.
    """
    seen: set[str] = set()
...
SPEC = OracleSpec(
    ...
    capabilities=[
        Capability(
            ...
            # Sells computation: the driven-dissipative relaxation is bounded by
            # MAX_TOPPLES (5M), and each topple does O(out_degree) work in pure Python, so
            # the real ceiling is tens of CPU-seconds. verify() replays the same pile, so
            # it carries the identical cost despite being 10x cheaper to buy.
            cost_ms=lambda d: min(
                sandpile.MAX_TOPPLES / 500.0,
                max(1, min(int(d.get("grains", 4000) or 4000), sandpile.MAX_GRAINS))
                * max(1, min(_n_from_request(d), sandpile.MAX_NODES))
                / 500.0,
            ),
