...
**RFC 9381 Appendix B.3 test vectors**: the tests assert that our implementation
reproduces the published `SK→PK`, the hash-to-curve point `H`, the proof
commitment `Gamma`, the VRF output `beta`, **and the full 80-byte proof `pi`**
byte-for-byte for all three vectors. The full-`pi` check pins the little-endian
`c || s` encoding mandated by RFC 9381 §5.5, so the proofs are byte-interoperable
with a conformant verifier (e.g. libsodium), not merely self-consistent; the tests
also confirm the RFC's own published `pi` bytes verify under our verifier. See
`tests/test_sortes.py`.
