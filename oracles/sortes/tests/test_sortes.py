...
#
# These are the values published in RFC 9381 Appendix B.3. The decisive anchors
...
# Full 80-byte proofs pi from the SAME RFC 9381 Appendix B.3 examples (16, 17, 18).
# Unlike H/Gamma/beta (which are independent of the c||s encoding), pi pins the
# challenge c and scalar s — the exact bytes a *conformant* ECVRF verifier
# (e.g. libsodium / the CFRG reference) checks. These match ONLY when
# int_to_string/string_to_int use the RFC-mandated little-endian convention
# (RFC 9381 §5.5). A big-endian implementation stays self-consistent and still
# reproduces Gamma and beta, but emits a byte-reversed c||s tail, so this test
# FAILS for it — that is the byte-interoperability regression guard.
RFC9381_TAI_PI = [
    ...
    ("4ccd089b28ff96da9db6c346ec114e0f5b8a319f35aba624da8cf6ed4fb8a6fb", bytes([0x72]),
    ...
    ("c5aa8df43f9f837bedb7442f31dcb7b166d38535076f094b85ce3a2e0b4458f7", bytes([0xAF, 0x82]),
...
class TestRFC9381FullProof:
    """Byte-interoperability: the full 80-byte pi must match RFC 9381 Appendix B.3."""

    @pytest.mark.parametrize("sk,alpha,pi", RFC9381_TAI_PI)
    def test_prove_full_pi_matches_rfc(self, sk, alpha, pi):
        # The decisive byte-conformance check. Fails under a big-endian c||s
        # encoding even though Gamma/beta still match; passes only for the
        # RFC-mandated little-endian encoding (RFC 9381 §5.5).
        assert vrf.prove(bytes.fromhex(sk), alpha).hex() == pi

    @pytest.mark.parametrize("sk,alpha,pi", RFC9381_TAI_PI)
    def test_rfc_reference_pi_verifies(self, sk, alpha, pi):
        # The proof bytes as published by the RFC must verify under our verifier
        # (i.e. our verifier is conformant, not merely self-consistent).
        pk = vrf.sk_to_pk(bytes.fromhex(sk))
        assert vrf.verify(pk, alpha, bytes.fromhex(pi)) is not None
...
class TestVRFRoundtrip:
    ...
    def test_scalar_s_out_of_range_rejected(self):
        ...
        alpha = b"x"
        pi = bytearray(vrf.prove(self.SK, alpha))
        # s is little-endian per RFC 9381 §5.5; encode L+1 the same way so this
        # genuinely sets an out-of-range scalar that decode_proof must reject.
        pi[48:80] = (vrf.L + 1).to_bytes(32, "little")
        assert vrf.verify(pk, alpha, bytes(pi)) is None
...
class TestSortesApp:
    ...
    @pytest.mark.asyncio
    async def test_caller_supplied_sk_is_refused_by_default(self, monkeypatch):
        """A draw under a key the CALLER chose is worthless, so it must be refused.

        `sk` is not in the published input_schema and oracle_core does not validate
        input against the schema, so this handler is the only gate. Without it any
        caller gets a well-formed draw response echoing their own `public_key` —
        which makes any certificate that trusts the response's `public_key`
        forgeable by an entrant. Default-off, opt-in for RFC-vector reproduction.
        """
        monkeypatch.delenv("SORTES_ALLOW_CALLER_SK", raising=False)
        transport = ASGITransport(app=app)
        async with AsyncClient(transport=transport, base_url="http://t") as c:
            dr = (await c.post("/ai-market/v2/invoke", json={
                "capability_id": "sortes.draw@v1",
                "input": {
                    "alpha": "winner-draw-2026",
                    "sk": "01" * 32,
                },
            })).json()
        # The attacker's key must never appear, and the oracle must not have drawn.
        body = json.dumps(dr)
        assert vrf.sk_to_pk(bytes.fromhex("01" * 32)).hex() not in body
        assert dr.get("ok") is not True

    @pytest.mark.asyncio
    async def test_draw_with_dev_sk_reproduces_rfc_vector(self, monkeypatch):
        ...
        # beta — all RFC 9381 Appendix B.3 (Example 18) values.
        monkeypatch.setenv("SORTES_ALLOW_CALLER_SK", "1")
        transport = ASGITransport(app=app)
        async with AsyncClient(transport=transport, base_url="http://t") as c:
            dr = (await c.post("/ai-market/v2/invoke", json={
                "capability_id": "sortes.draw@v1",
