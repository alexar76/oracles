...
0x03) is implemented so the RFC 9381 Appendix B.3 test vectors reproduce
bit-for-bit — including the full 80-byte proof pi, which pins the little-endian
c||s encoding a conformant verifier checks (see tests/test_sortes.py).
"""
...
# --- scalar / integer string helpers (RFC 9381 §5.5) -----------------------
# For the edwards25519 ciphersuites RFC 9381 §5.5 fixes int_to_string /
# string_to_int to the LITTLE-endian convention of RFC 8032 §5.1.2. This must be
# little-endian: a big-endian implementation is self-consistent (its own verifier
# still accepts its proofs) and even reproduces Gamma and beta, but it emits the
# c||s tail of pi byte-reversed, so the 80-byte proof is NOT byte-interoperable
# with a conformant verifier (libsodium / the CFRG reference). See the full-pi
# RFC 9381 Appendix B.3 regression test in tests/test_sortes.py.
def int_to_string(x: int, length: int) -> bytes:
    """int_to_string — little-endian fixed-length octet string (RFC 9381 §5.5)."""
    return x.to_bytes(length, "little")


def string_to_int(b: bytes) -> int:
    """string_to_int — little-endian octet string to nonnegative integer (RFC 9381 §5.5)."""
