...
def _alpha_to_bytes(alpha: Any) -> bytes:
    """Interpret the client seed `alpha`: only a 'hex:'-prefixed string is decoded as raw
    bytes, everything else is UTF-8 encoded.

    The docstring used to also promise that a "pure-even-length hex string" would be
    decoded. It never was, and it must not be: a caller seeding with the literal word
    "deadbeef" means those eight characters, not those four bytes. Guessing by shape makes
    the wire format ambiguous, which is the one thing a VRF seed cannot afford — the proof
    binds to exact bytes. The prefix is the disambiguator, and `draw` now emits it."""
    if alpha is None:
...
def _draw(d: dict[str, Any]) -> dict[str, Any]:
    ...
    #
    # FAIL CLOSED. `sk` is absent from the published input_schema and the protocol
    # layer does not validate input against that schema (oracle_core hands the raw
    # dict to the handler), so before this gate any caller could obtain a perfectly
    # well-formed `sortes.draw` response computed under a key THEY chose, with the
    # response echoing their own derived `public_key`. Every downstream fairness
    # claim collapses at that point: a draw certificate that quotes `public_key`
    # from the response — rather than pinning a key published before the draw —
    # is forgeable by any entrant. Verified reachable on the live oracle before
    # this gate existed. Off unless an operator explicitly opts in.
    sk = _SK
    pk = _PK
    dev_sk = d.get("sk")
    if dev_sk is not None:
        if os.environ.get("SORTES_ALLOW_CALLER_SK", "0").strip() not in ("1", "true", "TRUE"):
            raise ValueError(
                "caller-supplied `sk` is refused: the draw is only meaningful under "
                "this oracle's own key. Set SORTES_ALLOW_CALLER_SK=1 to reproduce "
                "published RFC 9381 vectors in a dev or test environment."
            )
    ...
    return {
        "suite": SUITE_STRING,
        "public_key": pk.hex(),
        # `hex:`-prefixed, so feeding this response straight back into sortes.verify@v1
        # works. It used to emit a bare hex string, which _alpha_to_bytes UTF-8 encodes
        # rather than decodes — so verify hashed the 18 ASCII characters "746573742d…"
        # instead of the 9 bytes "test-seed" and answered valid:false for every honest
        # proof the oracle had just issued. The two halves disagreed on the wire format.
        "alpha": "hex:" + alpha_raw.hex(),
        "alpha_hex": alpha_raw.hex(),
        "gamma": gamma.hex(),
        "c": c.hex(),
        "s": s.hex(),
        "pi": pi.hex(),
