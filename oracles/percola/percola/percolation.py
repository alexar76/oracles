...
def robustness(curve: list[dict[str, float]]) -> float:
    """Resilience scalar in [0, 1]: normalised area under P_inf(f) over f∈[0,1].

    At removal fraction ``f`` at most ``n·(1−f)`` nodes survive, so pointwise
    ``P_inf(f) ≤ 1−f`` and the *raw* trapezoidal area is bounded above by
    ``∫₀¹(1−f) df = 0.5`` — never by 1.0. To make the scalar a genuine 0…1 measure
    we divide the raw area by that tight bound of 0.5: ``0`` = collapses instantly,
    ``1`` = indestructible (a complete graph, where ``P_inf(f) = 1−f`` exactly).
    """
    if len(curve) < 2:
        return 0.0
    area = 0.0
    ...
    # Normalise by the tight upper bound (0.5) so the scalar is a true 0..1 measure.
    # Clamp guards against the rounded curve marginally overshooting the bound.
    return round(min(1.0, area / 0.5), 4)
...
def verify(
    ...
    For the random attack the removal order is driven by the *committed* seed
    ``H(graph_commitment || nonce)`` — never by a caller-chosen seed. A ``seed``
    argument, if given, is only accepted as a redundant restatement: it must equal
    the committed value, otherwise the result is rejected (``valid=False`` with an
    ``error``). This is what keeps the random baseline non-grindable.
    """
    labels, _index, adj, edge_set, commitment = canonical_graph(nodes, edges)
    n = len(labels)
    ...
    if attack == "random":
        # The random-attack seed is *committed*, not a free parameter: it MUST be
        # H(graph_commitment || nonce). We always derive it here and, if the caller
        # also supplied a seed, require it to equal the committed value. Accepting an
        # arbitrary caller-chosen seed would make the "verifiable random attack"
        # grindable — a caller could fish for a flattering permutation and get it
        # certified as valid. The recomputation therefore uses the committed seed.
        used_seed = seed_int(commitment, str(nonce))
        if seed is not None:
            try:
                supplied_seed = int(str(seed), 16)
            except ValueError:
                supplied_seed = None
                seed_error = "seed is not valid 16-hex"
            if seed_error is None and supplied_seed != used_seed:
                seed_error = (
                    "supplied seed does not equal H(graph_commitment || nonce); "
                    "the random-attack seed is committed, not caller-chosen"
                )
    ...
    valid = (
        seed_error is None
        and f_c is not None
        and abs(float(f_c) - recomputed_fc) < 1e-9
    )
