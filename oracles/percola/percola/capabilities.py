    ...
    capabilities=[
        Capability(
            ...
            description=(
                ...
                "curves, a 0..1 robustness scalar (area under targeted P_inf, "
                "normalised by its tight 0.5 bound), the keystone node set, and a "
                "graph_commitment for trustless replay."
            ),
            handler=_threshold,
            input_schema={
                "type": "object",
            ...
            # This capability sells computation, so it needs the tiers.py cost controls
            # that 40 of the family's 42 do not. Work is samples × one DSU sweep, plus an
            # O(n²) greedy removal order, run twice for attack="both". At the caps
            # (2000 nodes / 20000 edges / 200 samples) that is ~10 CPU-seconds of pure
            # Python — enough for one client to monopolise a worker, which is exactly
            # what these budgets exist to stop.
            cost_ms=lambda d: min(
                10_000.0,
                (
                    max(2, min(int(d.get("samples", 50) or 50), percolation.MAX_SAMPLES))
                    * (len(d.get("edges") or []) + len(d.get("nodes") or []) + 1)
                )
                / 440.0
                + (len(d.get("edges") or []) ** 2) / 100_000.0,
            ),
        ...
        Capability(
            ...
            p50_latency_ms=20,
            success_rate_30d=0.999,
            # verify REPLAYS the sweep, so it costs the same as producing it — being
            # 10x cheaper to buy makes it the better DoS lever, not the worse one.
            cost_ms=lambda d: min(
                10_000.0,
                (
                    max(2, min(int(d.get("samples", 50) or 50), percolation.MAX_SAMPLES))
