...
class TestThreshold:
    ...
    def test_robustness_is_a_true_unit_scale(self):
        # P_inf(f) <= 1 - f, so the RAW area is bounded by 0.5; the scalar is
        # normalised by that bound to be a genuine 0..1 measure. A complete graph
        # (P_inf(f) = 1 - f exactly) is the "indestructible" endpoint -> ~1.0.
        # Before normalisation it read 0.5, contradicting the "1 = indestructible"
        # claim in the README/docs.
        n = 12
        complete = [[i, j] for i in range(n) for j in range(i + 1, n)]
        rob_complete = pc.analyze(None, complete, samples=n, attack="targeted")["robustness"]
        assert 0.0 <= rob_complete <= 1.0
        assert rob_complete > 0.95  # indestructible -> top of the scale, not 0.5
        # The fragile star never exceeds the unit bound either.
        rob_star = pc.analyze(None, star(n), samples=n, attack="targeted")["robustness"]
        assert 0.0 <= rob_star <= 1.0
...
class TestVerify:
    ...
    def test_random_verify_with_committed_seed(self):
        # A supplied seed is accepted only as a redundant restatement of the
        # committed value H(commitment || nonce) — so the nonce must be given too.
        out = pc.analyze(None, ring(20), samples=20, attack="random", nonce="abc")
        v = pc.verify(None, ring(20), attack="random", f_c=out["random"]["f_c"],
                      nonce="abc", seed=out["random"]["seed"], samples=20)
        assert v["valid"] is True
        assert "error" not in v

    def test_random_verify_derives_seed_from_nonce(self):
        # No seed at all: the verifier derives it from the nonce and validates.
        out = pc.analyze(None, ring(20), samples=20, attack="random", nonce="abc")
        v = pc.verify(None, ring(20), attack="random", f_c=out["random"]["f_c"],
                      nonce="abc", samples=20)
        assert v["valid"] is True
        assert v["seed"] == out["random"]["seed"]

    def test_random_verify_rejects_grindable_seed(self):
        # Security regression: the "verifiable random attack" must be non-grindable.
        # An attacker grinds an arbitrary seed (NOT H(commitment || nonce)) that
        # yields a flattering order, then submits that seed plus the f_c it truly
        # produces. Before the fix verify() replayed the caller's seed and certified
        # it valid; now the uncommitted seed is rejected.
        edges = ring(20)
        labels, _idx, adj, edge_set, commitment = pc.canonical_graph(None, edges)
        n = len(labels)
        committed = pc.seed_int(commitment, "0")
        attacker_seed = (committed ^ 0xDEADBEEFDEADBEEF) & 0xFFFFFFFFFFFFFFFF
        assert attacker_seed != committed
        _curve, attacker_fc = pc.sweep(pc.seeded_order(n, attacker_seed),
                                       adj, edge_set, n, 20)
        v = pc.verify(None, edges, attack="random", f_c=attacker_fc,
                      seed=format(attacker_seed, "016x"), samples=20)
        assert v["valid"] is False
        assert "error" in v
        # And the verifier reports the committed seed, not the attacker's.
        assert v["seed"] == format(committed, "016x")
