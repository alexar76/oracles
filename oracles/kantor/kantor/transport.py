...
def solve_exact(a: np.ndarray, b: np.ndarray, C: np.ndarray) -> dict[str, Any]:
    ...
    # Zero-mass atoms (a_i == 0 or b_j == 0) carry no mass, so min-cost flow never routes
    # through their nodes and leaves those node potentials UNPINNED — the returned dual is
    # then not complementary-slack, so an honest certificate would fail verify() (dual
    # objective < cost). Solve the strictly-positive sub-problem (clean, tight duals) and
    # re-embed the zero-mass atoms with dual-FEASIBLE potentials: with mass 0 they never
    # change the dual objective, they only need u_i + v_j <= C_ij to hold everywhere.
    nz_i = np.flatnonzero(a > 0.0)
    nz_j = np.flatnonzero(b > 0.0)
    if nz_i.size != m or nz_j.size != n:
        # a, b are normalised upstream so at least one entry of each is > 0.
        sub = solve_exact(a[nz_i], b[nz_j], C[np.ix_(nz_i, nz_j)])
        P = np.zeros((m, n), dtype=float)
        flow_int = np.zeros((m, n), dtype=np.int64)
        P[np.ix_(nz_i, nz_j)] = sub["plan"]
        flow_int[np.ix_(nz_i, nz_j)] = sub["flow_int"]
        u = np.zeros(m, dtype=float)
        v = np.zeros(n, dtype=float)
        u[nz_i] = sub["u"]
        v[nz_j] = sub["v"]
        zi = np.setdiff1d(np.arange(m), nz_i)
        zj = np.setdiff1d(np.arange(n), nz_j)
        # dropped sinks first: tightest value feasible against every retained source;
        for j in zj:
            v[j] = float(np.min(C[nz_i, j] - u[nz_i]))
        # dropped sources next: feasible against ALL sinks (retained + the ones just set),
        # which also covers dropped-source / dropped-sink pairs.
        for i in zi:
            u[i] = float(np.min(C[i, :] - v))
        return {
            "plan": P,
            "flow_int": flow_int,
            ...
            "u": u,
            "v": v,
            "dual_objective": float(np.dot(a, u) + np.dot(b, v)),
            "flow": int(sub.get("flow", 0)),
            "supply": quantise(a, Q),
            "demand": quantise(b, Q),
        }
...
def transport(d: dict[str, Any]) -> dict[str, Any]:
    ...
    if out_method == "sinkhorn-approx":
        ...
        out["certificate"] = {
            "kind": "kantorovich-lp-primal-dual-optimality",
            "primal_feasibility": (
                "P >= 0, row-sums(P) == a, col-sums(P) == b, and <P,C> == cost "
                "(a feasible plan => cost is an UPPER bound on the optimum)"
            ),
            "dual_feasibility": (
                "u_i + v_j <= C_ij for every (i,j), and sum_i a_i*u_i + sum_j b_j*v_j == cost "
                "(a feasible dual => cost is a LOWER bound on the optimum)"
            ),
            "note": (
                "BOTH sides are required: a dual-only witness only lower-bounds the optimum, "
                "so it cannot rule out an under-reported cost. A feasible primal plan (upper "
                "bound) plus a feasible dual (lower bound), both equal to cost, pin the exact "
                "global optimum. Checkable in O(m*n) via kantor.verify (pass BOTH plan and "
                "potentials), no re-solve."
            ),
        }
    return out
...
def verify(d: dict[str, Any]) -> dict[str, Any]:
    """Trustless primal-dual optimality check — the ``kantor.verify@v1`` handler core.

    A dual certificate alone is NOT sufficient: for any dual-feasible ``(u, v)`` LP weak
    duality gives ``sum_i a_i*u_i + sum_j b_j*v_j <= optimal_cost`` — a LOWER bound only.
    So a dual-only check (feasibility + ``claimed_cost == dual_objective``) accepts any
    ``claimed_cost`` equal to a feasible-but-suboptimal dual value, i.e. it cannot detect
    an UNDER-reported cost (with ``u = v = 0`` the dual is feasible whenever ``C >= 0`` and
    its objective is 0, so a dishonest oracle could "prove" a cost of 0, or even negative).

    A complete LP optimality certificate therefore needs BOTH sides. Given ``a, b, C``
    (or points), a ``claimed_cost``, the primal transport plan ``P`` and the Kantorovich
    potentials ``(u, v)``, we confirm optimality in O(m*n) WITHOUT re-solving:

      * PRIMAL FEASIBILITY  ``P >= 0``, ``row-sums(P) == a``, ``col-sums(P) == b`` — a
        feasible plan, so ``<P,C>`` is an UPPER bound on the optimum — and
        ``<P,C> == claimed_cost``;
      * DUAL FEASIBILITY     ``u_i + v_j <= C_ij + tol`` for ALL i, j (the only place the
        whole cost matrix is touched — no shortcut exists), giving the dual objective as a
        LOWER bound, and STRONG DUALITY ``claimed_cost == sum_i a_i*u_i + sum_j b_j*v_j``.

    Upper bound == lower bound == ``claimed_cost`` pins the exact optimum (and a transport
    cost is >= 0, so ``claimed_cost < -tol`` is rejected outright). Returns
    ``{valid, feasible, strong_duality, primal_feasible, primal_cost_matches, ...}``.
    """
    a = _normalise_weights(_as_float_vec(d.get("a"), "a"), "a")
    b = _normalise_weights(_as_float_vec(d.get("b"), "b"), "b")
    ...
    if u.size != m or v.size != n:
        raise ValueError("potential lengths must match len(a) and len(b)")

    # The primal plan is REQUIRED — without it the certificate is dual-only and cannot
    # bound the optimum from above (see docstring), so an under-report would pass.
    if d.get("plan") is None:
        raise ValueError(
            "missing 'plan' (the primal transport plan P; required for a two-sided "
            "optimality certificate — a dual-only check cannot detect an under-reported cost)"
        )
    try:
        P = np.asarray(d["plan"], dtype=float)
    except (TypeError, ValueError):
        raise ValueError("'plan' must be a numeric m*n matrix")
    if P.ndim != 2 or P.shape != (m, n):
        raise ValueError(f"'plan' shape {P.shape} != (len(a)={m}, len(b)={n})")
    if not np.all(np.isfinite(P)):
        raise ValueError("'plan' must be finite")

    if d.get("claimed_cost") is None:
        raise ValueError("missing 'claimed_cost'")
    claimed_cost = float(d["claimed_cost"])

    # Cost-unit comparisons scale with the cost magnitude so they are robust to input
    # units; marginal / non-negativity checks are in probability units (order 1) so they
    # use the un-scaled tolerance — keeping primal feasibility tight regardless of max|C|.
    scale = max(1.0, float(np.max(np.abs(C))) if C.size else 1.0)
    base_tol = float(d.get("tol", 1e-6))
    cost_tol = base_tol * scale
    mtol = base_tol

    # --- DUAL side (lower bound): u_i + v_j <= C_ij on every pair (the O(m*n) sweep) --
    violation = (u[:, None] + v[None, :]) - C  # > 0 means infeasible
    max_violation = float(violation.max()) if violation.size else 0.0
    feasible = max_violation <= cost_tol
    dual_obj = float(np.dot(a, u) + np.dot(b, v))
    duality_gap = abs(claimed_cost - dual_obj)
    duality_ok = duality_gap <= cost_tol

    # --- PRIMAL side (upper bound): P is a feasible plan and <P,C> == claimed_cost ----
    min_plan = float(P.min()) if P.size else 0.0
    nonneg_ok = min_plan >= -mtol
    row_violation = float(np.max(np.abs(P.sum(axis=1) - a))) if m else 0.0
    col_violation = float(np.max(np.abs(P.sum(axis=0) - b))) if n else 0.0
    marginal_violation = max(row_violation, col_violation)
    marginals_ok = marginal_violation <= mtol
    primal_feasible = bool(nonneg_ok and marginals_ok)
    primal_cost = float(np.sum(P * C))
    primal_gap = abs(primal_cost - claimed_cost)
    primal_cost_ok = primal_gap <= cost_tol

    # A transport cost is >= 0 (C >= 0, mass >= 0); a negative claim is invalid outright.
    nonneg_cost_ok = claimed_cost >= -cost_tol

    valid = bool(
        feasible and duality_ok
        and primal_feasible and primal_cost_ok
        and nonneg_cost_ok
    )
    return {
        "valid": valid,
        ...
        "primal_cost_matches": bool(primal_cost_ok),
        "dual_objective": round(dual_obj, 12),
        ...
        "primal_gap": round(primal_gap, 12),
        "marginal_violation": round(marginal_violation, 12),
        "min_plan_entry": round(min_plan, 12),
        "m": int(m),
        "n": int(n),
        "note": (
            "TWO-SIDED certificate: a feasible primal plan (P>=0, marginals==a,b) makes "
            "<P,C> an upper bound and a feasible dual (u_i+v_j<=C_ij) makes sum a_i u_i + "
            "sum b_j v_j a lower bound; both equal to claimed_cost => claimed_cost is the "
            "exact optimum. A dual-only check would accept an under-reported cost. Checked "
            "in O(m*n), no re-solve, no trust in the oracle."
        ),
    }
