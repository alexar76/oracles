...
# maxItems mirrors transport.MAX_BINS, which the solver already enforces — it just was not
# published, so a caller reading the manifest had no way to learn the limit except by
# being refused. Exact min-cost flow is O(m*n) in memory before it is anything in time.
_VEC = {"type": "array", "items": {"type": "number"}, "maxItems": transport.MAX_BINS}
_COST_SCHEMA = {
    "type": "array",
    "description": "Explicit m*n ground-cost matrix C (non-negative). Mutually exclusive with points.",
    "items": _VEC,
    "maxItems": transport.MAX_BINS,
}
_POINTS_SCHEMA = {
    "type": "array",
