...
> **Degenerate bound.** If coincident input points drive the lower bound to `0`, it
> certifies nothing, so `gap` is reported as `null` (unknown) rather than a spurious
> `0`. The one exception is all-coincident points, where `length` is also `0`: the
> tour is then provably optimal and `gap` is a genuine `0`.
...
Output: `{ "tour": [int], "length": float, "lower_bound": float, "gap": float | null, "n": int }` (`gap` is `null` when a zero lower bound certifies nothing)
