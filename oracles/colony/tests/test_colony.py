...
class TestTSP:
    ...
    def test_zero_lower_bound_does_not_fabricate_optimality_certificate(self):
        # Two clusters of coincident duplicates: every node has a zero-cost incident
        # edge, so the lower bound collapses to 0 while the tour still has positive
        # length (it must cross between the clusters). A gap of 0 here would falsely
        # certify the tour as provably optimal from a bound that certifies nothing.
        # The oracle must instead report the gap as unknown (None).
        pts = [[0.0, 0.0], [0.0, 0.0], [10.0, 10.0], [10.0, 10.0]]
        r = tsp.solve(pts, iterations=1000)
        assert r["lower_bound"] == 0.0
        assert r["length"] > 0.0
        assert r["gap"] is None  # was a spurious 0.0 before the fix

    def test_all_coincident_points_are_trivially_optimal(self):
        # All points identical -> every tour has length 0 == optimum, so a 0 gap is a
        # *genuine* certificate (handled explicitly, not via the unsound lb==0 fallback).
        r = tsp.solve([[2.0, 2.0]] * 5, iterations=1000)
        assert r["lower_bound"] == 0.0
        assert r["length"] == 0.0
        assert r["gap"] == 0.0

    def test_positive_bound_gap_is_a_real_number(self):
        # Sanity: with distinct points the certificate is a concrete non-negative gap.
        r = tsp.solve(_square(), iterations=1000)
