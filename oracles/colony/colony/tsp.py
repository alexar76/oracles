...
def solve(points: list[list[float]] | np.ndarray, iterations: int = 1000) -> dict[str, Any]:
    ...
    The gap is the certificate ``(length - lower_bound) / lower_bound``. It is only
    meaningful when the lower bound is strictly positive. When coincident points
    drive the bound to ``0`` (every node has a zero-cost incident edge) the bound
    carries no information about the optimum, so:

    * if the tour also has length ``0`` (all points coincide) the tour is provably
      optimal and ``gap`` is a genuine ``0.0``;
    * otherwise the optimality gap is **unknown** and reported as ``None`` — the
      oracle refuses to emit a spurious ``0``-gap optimality certificate it cannot
      back with a positive lower bound.
    """
    ...
    if lb > 0:
        ...
        # Fully degenerate: all points coincide, so every tour has length 0. The
        # optimum is provably 0 == length, hence a genuine 0-gap certificate.
        gap = 0.0
    else:
        # Coincident duplicates zeroed the lower bound (each node has a zero-cost
        # incident edge) while the tour still has positive length. The bound then
        # certifies nothing, so the optimality gap is unknown — do NOT fabricate a
        # 0-gap optimality certificate we cannot back with a positive bound.
        gap = None
